    ...
    <div class="container-fluid px-4">
        ...
        <form method="POST" action="{{ route('descuento-actualizar', $descuento->id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" placeholder="Ingrese solo numeros, sin el %" name="cantidad" class="form-control @error('cantidad') is-invalid @enderror" aria-describedby="emailHelp" value="{{ old('cantidad') == "" ? $descuento->cantidad : old('cantidad') }}">
                @error('descuento')
                  <small class="text-danger"> {{ '*'.$message}}</small>
              @enderror
            </div>
